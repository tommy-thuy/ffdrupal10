...
class AjaxController extends ControllerBase {
   ...
   * @param \Symfony\Component\Mime\MimeTypeGuesserInterface $mimeType
  ...
  private function createFileKey($directory, $file_name) {
    ...
    $public_folder = 's3://' . rtrim(($config->get('public_folder') ?: 's3fs-public'), '/');
    $private_folder = 's3://' . rtrim(($config->get('private_folder') ?: 's3fs-private'), '/');
    if (strpos($uri_base, $public_folder) === 0 || strpos($uri_base, $private_folder) === 0) {
      $uri_base = str_replace([$public_folder, $private_folder], ['public:/', 'private:/'], $uri_base);
    }
  ...
  public function saveFile($file_path, $file_name, $file_size, $field_name) {
    ...
    // the file is saved so the filesize can be found from the cache.
