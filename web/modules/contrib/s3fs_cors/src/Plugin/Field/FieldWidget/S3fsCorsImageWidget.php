    ...
    if ($submitted_value) {
      ...
      if (preg_match('/(\d+)(.*)/', $submitted_value, $matches)) {
        ...
        if ($suffix && !in_array($suffix, ['B', 'KB', 'MB', 'GB'])) {
          $form_state->setError($element, $this->t('Invalid numeric value or size suffix. Specify an integer followed by "KB", "MB" or "GB".'));
          return;
        }
      }

      $max_filesize = Bytes::toNumber($submitted_value);
      if (!$max_filesize) {
        $form_state->setError($element, $this->t('Invalid max filesize. Enter a value like "512" (bytes), "80 KB" (kilobytes), "50 MB" (megabytes) or "2 GB" (gigabytes).'));
      }

      if ($max_filesize > Bytes::toNumber('5 GB')) {
        $form_state->setError($element, $this->t('Invalid max filesize. 5 GB is largest file size current supported.'));
      }
    }
