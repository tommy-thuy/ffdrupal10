...
(function ($, Drupal, once) {
  ...
  Drupal.behaviors.s3fsCorsAutoUpload = {
    ...
    attach: function (context, settings) {
      ...
      $(once.remove('auto-file-upload', '.s3fs-cors-file input[type="file"]', context)).off('.autoFileUpload');
      $(once.remove('auto-file-upload', 'input.s3fs-cors-upload', context)).off('.autoFileUpload');
      // Attach the custom s3fs cors auto upload processing behavior.
      $(once('s3fs-cors-auto-upload', '.s3fs-cors-file input[type="file"]')).on('change.s3fsCorsAutoUpload', {settings: settings.s3fs_cors, baseUrl: settings.path.baseUrl}, Drupal.s3fsCors.triggerUploadButton);
      $(once('s3fs-cors-auto-upload', 'input.s3fs-cors-upload')).on('change.s3fsCorsAutoUpload', {settings: settings.s3fs_cors, baseUrl: settings.path.baseUrl}, Drupal.s3fsCors.triggerUploadButton);
    },
    detach: function (context, settings, trigger) {
      if (trigger === 'unload') {
        $(once.remove('s3fs-cors-auto-upload', '.s3fs-cors-file input[type="file"]', context)).off('.s3fsCorsAutoUpload');
      }
    }
  };
  ...
  Drupal.s3fsCors = Drupal.s3fsCors || {
    ...
    triggerUploadButton: function (event) {
      ...
      for (var fix = 0; fix < num_files; fix++) {
        ...
        var ajax_uri = baseUrl + 'ajax/s3fs_cors_key/' + encodeURIComponent(directory) + '/' + encodeURIComponent(file_obj.name) + '/' + file_obj.size + '/' + fix;
        $.get(ajax_uri, function (data, status) {
          ...
          $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
              ...
              var ajax_uri = baseUrl + 'ajax/s3fs_cors_save/' + encodeURIComponent(file_key) + '/' + encodeURIComponent(file_name) + '/' + file_size + '/' + field_name;
              $.get(ajax_uri, function (data, status) {
                if (!data.fid) {
                  if (data.errmsg) {
                    alert(data.errmsg);
