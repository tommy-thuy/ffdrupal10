...
class JsonLog implements LoggerInterface {
  ...
  public function log($level, $message, array $context = []): void {
    ...
    if (!file_put_contents($this->file, $prepend . $log_entry->getJson() . $append, FILE_APPEND | LOCK_EX)) {
  ...
  public function prepareLog($level, $message, array $context = []) {
    ...
    // Filter out non-whitelisted log channels.
    if ($this->channels) {
  ...
  public function getFileName($file_time_format) {
    return $this->dir . '/' . $this->site_id . ((empty($file_time_format) || $file_time_format == 'none') ? '' : ('.' . date($file_time_format))) . '.json.log';
  }
  ...
  private function loadDefaultSettings() {
    ...
    // Channels
    $channels = getenv('drupal_jsonlog_channels') ?: $this->config->get('jsonlog_channels');
    $this->channels = $channels ? explode(',', $channels) : [];
    ...
    if (!($this->stdout = getenv('drupal_jsonlog_stdout'))) {
      $this->stdout = $this->config->get('jsonlog_stdout') ?? FALSE;
    }
